...
class TestTimberFormatter(unittest2.TestCase):
    def setUp(self):
        self.context = TimberContext()
        self.customer = {'id': '1'}
    ...
    def test_format_emits_single_line(self):
        formatter = timber.TimberFormatter(context=self.context)
    ...
    def test_format_with_custom_default_json_serializer(self):
        ...
        default_formatter = timber.TimberFormatter(context=self.context)
        default_logger, _ = logger_and_lines(default_formatter, 'default')

        suppress_formatter = timber.TimberFormatter(context=self.context, json_default=suppress_encoding_errors)
    ...
    def test_format_with_custom_default_json_encoder(self):
        ...
        dummy_capable_formatter = timber.TimberFormatter(context=self.context, json_encoder=DummyCapableEncoder)
