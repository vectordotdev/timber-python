...
class TestFlushWorker(unittest2.TestCase):
    ...
    @mock.patch('timber.flusher._calculate_timeout')
    def test_flushes_after_interval(self, calculate_timeout):
        ...
        def timeout(last_flush, interval):
            ...
            if self.timeout_calls < num_items:
                return 1000000
            return 0.0001
        calculate_timeout.side_effect = timeout
    ...
    @mock.patch('timber.flusher._calculate_timeout')
    @mock.patch('timber.flusher._initial_timeout')
    def test_does_nothing_without_any_items(self, initial_timeout, calculate_timeout):
        print(self)
        calculate_timeout.side_effect = lambda a,b: 0.0001
        initial_timeout.side_effect = lambda a: 0.0001
    ...
    @mock.patch('timber.flusher.time.sleep')
    ...
    @mock.patch('timber.flusher.sys.exit')
